...
__VERSION__ = "0.1.2"
__SESSIONCONFIGFILE__ = "config" + os.sep + "sessions"
...
class ControlMainWindow(QtGui.QMainWindow):
	...
	def writeConfig(self):
		try:
			...
			for id, session in enumerate(self.sessions):
				sessionCfg[id] = { "path":session.path, "session":session.session, "repo":session.repo }
			with open(__SESSIONCONFIGFILE__, 'w+') as file:
	...
	def checkConfig(self):
		try:
			if not os.path.exists(os.path.dirname(__SESSIONCONFIGFILE__)):
				os.makedirs(os.path.dirname(__SESSIONCONFIGFILE__))
			self.writeConfig()
	...
	def readConfig(self):
		try:
			with open(__SESSIONCONFIGFILE__) as file:
				sessionCfg = json.load(file)
			for line in range(len(sessionCfg)):
				session = BoarSession(sessionCfg[str(line)]["path"], sessionCfg[str(line)]["session"], sessionCfg[str(line)]["repo"])
	...
	def loadDirList(self):
		...
		for session in self.sessions:
			...
			# direction of the list ist set to rtl, to get the icon to the right.
			# therefore textlignment has to be set to right, to get the text to the left ... strange
	...
	def folders(self):
		...
		dir = QtGui.QFileDialog.getExistingDirectory(self, "Open Directory", os.getcwd(), flags)
		self.addFolder(dir)
		...
		if dir != "":
			try:
				...
				if self.ui.dirList.findItems(dir, findflags):				
					return
				session = BoarSession(dir, boarCfg["session_name"], boarCfg["repo_path"])
				self.sessions.append(session)
				self.writeConfig()
				self.loadDirList()
	...
	def editSession(self):
		...
		if selectedSession == -1:
			return
		params = [self.sessions[selectedSession].session, "ignore", "--repo=" + self.sessions[selectedSession].repo]
	...
	def getPropsFinished(self, data):
		...
		if selectedSession == -1:
			return
		self.loading(False)
		ret = self.ui.showSessionDialog(self, self.sessions[selectedSession].path, self.sessions[selectedSession].session, self.sessions[selectedSession].repo, ignoreList)
		if ret["code"] == QtGui.QDialog.Accepted:
			params = [self.sessions[selectedSession].session, "ignore", ret["ignore"], "--repo=" + self.sessions[selectedSession].repo]
