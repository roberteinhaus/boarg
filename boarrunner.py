...
class BoarRunner(QtCore.QThread):
	...
	def run(self):
		try:
			...
			if self.path != "":	
				p = subprocess.Popen(processCmd, cwd=self.path, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			else:
				p = subprocess.Popen(processCmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			progress += 5
			...
			while True:
				...
				if nextline != "":
					ret.append(nextline)
					if "Looking" in nextline or "Verifying" in nextline or "Scanning" in nextline or "Sending" in nextline:
